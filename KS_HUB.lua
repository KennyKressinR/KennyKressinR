-- KS HUB v0.2.3 - Unificado y optimizado (base v0.2.1 + TP list + toggles)
...
local closeY = (HEADER_H - 32) / 2
closeBtn.Position = UDim2.new(1, -40, 0, closeY)
...
-- Scrolling frames (contenido, espaciamiento reducido)
...
-- Helpers: sección y grid simétrico (2 columnas)
...
local function setTrans(v) main.BackgroundTransparency = v end

-- Teleport function (mantengo compatibilidad parcial/nombre/display)
local function tpTo(name)
    ...
    if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if char and char:FindFirstChild("HumanoidRootPart") then
            -- offset up a bit so you don't get stuck inside
...
-- Visuales (highlight & light)
local function addHL(p)
    if not p or not p.Character or playersHL[p] then return end
    local ok,hl = pcall(function() local h = Instance.new("Highlight"); h.Name = "KS_Highlight"; h.Parent = p.Character; return h end)
    if ok and hl then playersHL[p] = hl end
end
local function removeHL(p)
    if playersHL[p] then pcall(function() playersHL[p]:Destroy() end); playersHL[p] = nil
    else
...
local function togglePlayersView(s)
    ...
    if s then
        ...
        playersAddConn = Players.PlayerAdded:Connect(function(p) if p ~= pl then p.CharacterAdded:Connect(function() addHL(p) end); addHL(p) end end)
...
local function toggleLight(s)
    ...
    if s then
        if hrp and not hrp:FindFirstChild("KS_PointLight") then
            local plight = Instance.new("PointLight"); plight.Name = "KS_PointLight"; plight.Range = 16; plight.Brightness = 2; plight.Parent = hrp; lightInst = plight
        end
    else
        if hrp and hrp:FindFirstChild("KS_PointLight") then
            pcall(function() hrp:FindFirstChild("KS_PointLight"):Destroy() end); lightInst = nil
        end
    end
end

-- Construcción del contenido (simétrico + compacto)
do
    local p = scMain
    ...
    tb.Text = ""                            -- no texto prellenado
    tb.PlaceholderText = "Nombre del jugador" -- texto "fantasma"
    tb.Font = Enum.Font.Gotham
    tb.TextSize = 15
    tb.ClearTextOnFocus = true              -- borra automáticamente si tiene texto
    tb.ZIndex = 3
...
do
    ...
    grid(p, {
        { text = "Normal", cb = function() setTrans(0) end },
        { text = "50%", cb = function() setTrans(0.5) end },
    })
...
closeBtn.MouseButton1Click:Connect(function()
    ...
    reopen.Size = UDim2.new(0,120,0,34)
    reopen.Position = UDim2.new(0.5, -60, 0, 10)
    reopen.Text = "Open Hub"
    reopen.Font = Enum.Font.GothamBold
    reopen.TextSize = 16
    reopen.TextColor3 = Color3.new(1,1,1)
    reopen.BackgroundColor3 = Color3.fromRGB(0,140,255)
